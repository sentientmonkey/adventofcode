...
(define (parse-instructions str)
  (for/list ([ins (map string-split (string-split str "\n"))])
            (match ins
              [(list r1 op v1 "if" r2 cmp v2)
               (list (list r1 op (string->number v1))
                     (list r2 cmp (string->number v2)))])))


(define-simple-macro (compare oper env reg val)
  (oper (hash-ref env reg 0) val))

(define (eval-cond env condition)
  (match condition
    [(list reg "<" val) (compare < env reg val)]
    [(list reg ">" val) (compare > env reg val)]
    [(list reg ">=" val) (compare >= env reg val)]
    [(list reg "<=" val) (compare <= env reg val)]
    [(list reg "==" val) (compare equal? env reg val)]
    [(list reg "!=" val) (not (compare equal? env reg val))]))

(define-simple-macro (operand op env reg val)
  (hash-update env reg (λ (x) (op x val)) 0))

(define (eval-operand env oper)
  (match oper
...
(module+ test
  (require rackunit)
  [check-equal? (cpu-max "b inc 5 if a > 1") 0]
  [check-equal? (cpu-max "a inc 1 if b < 5") 1]
  [check-equal? (cpu-max "b inc 5 if a > 1\na inc 1 if b < 5\nc dec -10 if a >= 1\nc inc -20 if c == 10")
                10])


(cpu-max "yxr inc 119 if nev != 6
piw dec -346 if tl != 4
