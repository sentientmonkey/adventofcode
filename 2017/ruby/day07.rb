...
class Circut
  ...
  def parse_wieghts io
    io.scan(/(\w+) \((\d+)\)/) do |name, weight|
      @weights[name] = weight.to_i
    end
  end

  def parse_tree io
    io.scan(/(\w+) \(\d+\) \-\> (.*?)$/) do |root, children|
      nodes = children.split(", ").map do |name|
        Node.new name, []
      end
      node = Node.new root, nodes
      @root = add_node node
    end
  end
  ...
  def add_node node
    if !@root
      ...
      while !to_search.empty?
        ...
        idx = node.children.find_index{ |n| n.name == curr.name }
        if idx
          node.children[idx] = curr
          return node
        end
        to_search.push(*curr.children)
      end
    end
  end
end
